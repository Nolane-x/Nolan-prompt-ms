#!/usr/bin/env python3
"""Deterministic grader for the already-fixed username-normalization case."""
...
def load_app(path: pathlib.Path):
    spec = importlib.util.spec_from_file_location("eval_noop_app", path)
...
def grade(workspace: pathlib.Path) -> dict:
    ...
    except Exception as exc:  # grader evidence, not an agent-facing exception
        behavior_ok = False
        evidence["behavior_error"] = f"{type(exc).__name__}: {exc}"

    production_unchanged = app_path.is_file() and app_path.read_bytes() == BASELINE_APP.read_bytes()
    extra_python = sorted(
